...
Script Date: 2022-10-14

Description:
 
For data contained in tables t_k (k = 1, ... , kmax)
whose structure is identical to that of wave_available_vehicles,
...
- the values retrieved from the columns vehicle_id_k
are inserted in separate columns t_k_vehicle_id of the comparison table.

*/

DROP PROCEDURE IF EXISTS cmpr_variants_wv;
DELIMITER $$
CREATE PROCEDURE cmpr_variants_wv(
	...
	READS SQL DATA
	COMMENT 'Compares variants of wave_available_vehicles'
...
BEGIN
	...
	BEGIN
	    ...
	    SELECT CONCAT(
	    	  'Error executing cmpr_variants_wv(): '
			, errno
	    	, " - "
	    	, msg
	    ) AS "Error message";
	...
	DROP TEMPORARY TABLE IF EXISTS id_cols_wv;
	CREATE TEMPORARY TABLE id_cols_wv (
		id INT,
		col_name VARCHAR(64)
	);
	INSERT INTO id_cols_wv 
	VALUES 
		  (1, 'wave_id')
	;

	DROP TEMPORARY TABLE IF EXISTS dat_cols_wv;
	CREATE TEMPORARY TABLE dat_cols_wv (
		id INT,
		col_name VARCHAR(64)
	);
	INSERT INTO dat_cols_wv 
	VALUES 
		  (1, 'vehicle_id')
	;
