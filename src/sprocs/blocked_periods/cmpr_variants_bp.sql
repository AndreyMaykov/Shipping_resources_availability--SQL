...
whose structure is identical to that of blocked_periods,
...
- the values retrieved from the columns user_id_k (k = 1, ..., kmax)
are gathered in the comparison table's column with the name user_id;
- the values retrieved from the columns period_beginning_k and period_end_k 
are inserted in separate columns t_k_period_beginning and t_k_period_end 
of the comparison table.

*/

DROP PROCEDURE IF EXISTS cmpr_variants_bp;
DELIMITER $$
CREATE PROCEDURE cmpr_variants_bp(
	...
	READS SQL DATA
	COMMENT 'Compares variants of blocked_periods'
...
BEGIN
	...
	BEGIN
	    ...
	    SELECT CONCAT(
	    	  'Error executing cmpr_variants_bp(): '
			, errno
	    	, " - "
	    	, msg
	    ) AS "Error message";
